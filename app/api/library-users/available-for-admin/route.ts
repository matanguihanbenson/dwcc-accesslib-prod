...
export async function GET(req: NextRequest) {
  try {
    ...
    if (token.role !== 'SUPER_ADMIN') {
      return NextResponse.json(
        { error: 'Insufficient permissions. Only SUPER_ADMIN can register admin accounts.' },
        { status: 403 }
      )
    }

    // Get users who currently have non-USER roles (ADMIN/STAFF/SUPER_ADMIN)
    // Users with role USER or with no account should be available for admin promotion
    ...
    // Then get all users who are NOT in the existing user accounts
    ...
    console.error('Error fetching library users for admin promotion:', error)
